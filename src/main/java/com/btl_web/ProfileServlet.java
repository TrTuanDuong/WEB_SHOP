...
@WebServlet(urlPatterns = {"/profile", "/profile/update", "/profile/address/add", "/profile/address/default", "/profile/address/update"})
public class ProfileServlet extends HttpServlet {
    ...
    private void updateProfile(HttpServletRequest request, HttpServletResponse response, UserStore.User currentUser)
        ...
        if (UserStore.isCheckoutProfileReady(latest)) {
            request.getSession().setAttribute("profileError", "Thông tin cá nhân cố định đã được xác lập. Muốn sửa vui lòng liên hệ admin.");
            response.sendRedirect(request.getContextPath() + "/profile");
            return;
        }
        ...
        if (fullName.isEmpty() || ageText.isEmpty() || gender.isEmpty() || email.isEmpty() || phone.isEmpty() || baseAddress.isEmpty()) {
